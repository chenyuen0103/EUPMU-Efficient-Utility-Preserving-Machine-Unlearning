...
#   <SAVE_DIR>/<ARCH>/<DATASET>/<FORGET_TAG>/RL/<METHOD>/
#       class_<CLS>/seed_<N>_train_<FIXED_TRAIN_SEED>/
#       ulr_<LR>_eta_<E>_rw_<R>_fw_<F>_rho_<RHO>_epoch_<EP>/
#
# Usage:
#   bash run_classwise_omd_sweep.sh
#
...
# keep forget_ref=0 and forget_loss_type=rl (defaults) — omitted from the tag
...
# ── Build fixed setting tag prefix (no fref / flt suffix) ────────────────────
...
echo "  setting   : ulr_<lr>_${setting_prefix}_rho_<rho>_epoch_${UNLEARN_EPOCHS}"
    ...
    for seed in "${SEED_LIST[@]}"; do
      for lr in "${LR_LIST[@]}"; do
        for rho in "${RHO_LIST[@]}"; do
          ...
          setting_tag="ulr_${lr_suffix}_${setting_prefix}_rho_${rho_suffix}_epoch_${UNLEARN_EPOCHS}"
          ...
          result_dir_parts=("$SAVE_DIR" "$ARCH" "$DATASET" "$FORGET_TAG" "RL" "$method")
          if [[ -n "$RUN_NAMESPACE" ]]; then
...
echo
echo "Class-wise OMD-TCH sweep completed."
