...
echo "EUPMU-specific hyperparameters in this repo:"
echo "  method variant: eu / eu_fast"
echo "  eu_w_lr: dynamic weighting learning rate"
echo "  eu_error: retain-tolerance term in the implicit weighting update"
echo "  classes: ${CLASSES}"
echo
echo "Fixed defaults mirrored from run_all_classwise.sh:"
echo "  eu uses unlearn_lr = ${EU_UNLEARN_LR}"
echo "  eu_fast uses unlearn_lr = ${EU_FAST_UNLEARN_LR}"
      ...
      for eu_w_lr in "${EU_W_LR_LIST[@]}"; do
        ...
        for eu_error in "${EU_ERROR_LIST[@]}"; do
          ...
          for epochs in "${EPOCH_LIST[@]}"; do
            ...
            setting_tag="ulr_${ulr_suffix}_wlr_${wlr_suffix}_err_${err_suffix}_epoch_${epochs}"
            ...
            # wandb_entity doubles as the save-dir suffix in main_random.py.
...
if (( ${#FAILURES[@]} > 0 )); then
  echo
  echo "Some EUPMU sweep jobs failed:" >&2
  printf '  %s\n' "${FAILURES[@]}" >&2
  exit 1
fi

echo
echo "EUPMU sweep completed."
