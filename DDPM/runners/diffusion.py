...
class Diffusion(object):
    ...
    def saliency_unlearn(self):
        ...
        model.load_state_dict(states[0], strict=True)
        optimizer = get_optimizer(config, model.parameters())
        weight_opt = EU(device="cuda:0", w_lr=wandb.config.weight_learning_rate_eu, error=wandb.config.error_eu)
        weight_opt.set_min_losses(torch.Tensor([0.0, 0.0]).to("cuda"))
        ...
        for step in range(0, self.config.training.n_iters):
            ...
            wandb.log({"forget_loss": forget_loss.item(), "remain_loss": remain_loss.item(), "eu_weight": weight_opt.w.item()})
            # MARK: losses
            if (step + 1) % self.config.training.log_freq == 0:
                end = time.time()
            ...
            optimizer.zero_grad()
            weight_opt.backward(torch.stack([remain_loss,forget_loss]), model.parameters())
            optimizer.step()
            
            with torch.no_grad():
                new_remain_loss = loss_registry_conditional[config.model.type](
                    model, remain_x, remain_t, remain_c, remain_e, remain_b
                )
                new_remain_loss = args.alpha * new_remain_loss
                weight_opt.update(new_remain_loss)
            ...
            if (step + 1) % self.config.training.snapshot_freq == 0:
                ...
                wandb.log({"forgetting_loss":forget_loss, "ewc_loss":remain_loss})
